...
curr_age = as.numeric(lubridate::today() - lubridate::make_date(1994,01,04)) / 365
curr_height = 70 * 2.54
curr_activity_factor = 1.55
curr_activity_adjustment = -798.95
...
Compute_Calorie_Control_Linear <- function(days_since_last_weighin,
  ...
  # Determine the percentage_adjustment_based on the mode that we are in
  # browser()
  stopifnot(program_mode %in% c("Slow", "Medium", "Aggressive", "Select Pace"))
  if(program_mode == "Slow") {
    percentage_adjustment_input = 0.04
  } else if(program_mode == "Medium") {
    percentage_adjustment_input = 0.08
  } else {
    percentage_adjustment_input = 0.12
  }
...
MensRosaBMR = function(body_weight, height, age) {
  stopifnot(body_weight < 120)
  stopifnot(body_weight > 80)
...
#%% Beginning of the  actual calculations
# 
# # Inputs --------------------------------------------
# working_date = today()
# goal_bf = 0.10
...
shinyServer(function(input, output) {
  ...
  calorie_update <- eventReactive(input$kgLossRate, {
    ...
    # Get time since the last checkin
    # browser()
    initdata = read.csv(file = "FLF_output_table.csv", header=TRUE, sep = ",")
    ...
    multiplier = NA
    if(input$kgLossRate == "Medium") {multiplier = 0.008}
    else if (input$kgLossRate == "Slow") {multiplier = 0.004}
    else {multiplier = 0.012}
    
    # Compute the delta to the calories
  ...
  observeEvent(input$kgLossRate,
               {
                 if(input$kgLossRate != "Select Pace") {
                   calorie_update()
                 }
               })
